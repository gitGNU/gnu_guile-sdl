 ...
 *  Time-stamp: <2001-06-10 22:35:27 foof>                         *
...
wm_set_caption (SCM title, SCM icon)
{
   SCM_ASSERT ((SCM_NIMP (title) && SCM_STRINGP (title)),
               title, SCM_ARG1, "set-caption");

   SCM_ASSERT ((SCM_NIMP (icon) && SCM_STRINGP (icon)),
               icon, SCM_ARG1, "set-caption");
...
wm_set_icon (SCM icon)
{
   ...
   SCM_ASSERT_SMOB (icon, surface_tag, SCM_ARG1, "set-icon");
...
wm_toggle_full_screen (SCM s_surface)
{
   ...
   if (s_surface == SCM_UNDEFINED) {
      ...
      SCM_ASSERT_SMOB (s_surface, surface_tag, SCM_ARG1, "toggle-full-screen");
...
wm_grab_input (SCM s_mode)
{
   ...
   if (s_mode != SCM_UNDEFINED) {
      SCM_ASSERT (SCM_INUMP (s_mode), s_mode, SCM_ARG1, "grab-input");
...
{
   /* functions */
   scm_c_define_gsubr ("set-caption",         2, 0, 0, wm_set_caption);
   scm_c_define_gsubr ("get-caption",         0, 0, 0, wm_get_caption);
   scm_c_define_gsubr ("set-icon",            1, 0, 0, wm_set_icon);
   scm_c_define_gsubr ("iconify-window",      0, 0, 0, wm_iconify_window);
   scm_c_define_gsubr ("toggle-full-screen",  0, 1, 0, wm_toggle_full_screen);
   scm_c_define_gsubr ("grab-input",          0, 1, 0, wm_grab_input);
